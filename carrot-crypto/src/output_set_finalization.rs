...
pub fn get_additional_output_type(
    ...
    if already_completed {
        ...
        // num_selfsend == 1 && need_change_output
        {
...
#[must_use]
fn check_normal_proposals_randomness(
    ...
    for i in 0..normal_payment_proposals.len() {
        for j in 0..normal_payment_proposals.len() {
            if i == j {
                continue;
            } else if normal_payment_proposals[i].randomness
                == normal_payment_proposals[j].randomness
            {
                return Err(Error::new(ErrorKind::MissingRandomness));
            }
        }
...
pub fn get_output_enote_proposals<VB, VI>(
    normal_payment_proposals: &[CarrotPaymentProposalV1],
    ...
    VI: ViewIncomingKeyDevice,
{
    ...
    // assert output sizes
    if output_enote_proposals_out.len() != num_proposals
        || payment_proposal_order_out.len() != num_proposals
    {
        return Err(Error::new(ErrorKind::WrongOutputNumber));
    }
    ...
    for i in 0..normal_payment_proposals.len() {
        ...
        if num_proposals == 2 {
            other_enote_ephemeral_pubkey =
                Some(output_enote_proposal.enote.enote_ephemeral_pubkey.clone());
        }
    ...
    if num_proposals == 2 && num_selfsend_proposals == 2 {
        let use_ephem_pk_idx = if selfsend_payment_proposals[0]
        ...
        other_enote_ephemeral_pubkey = selfsend_payment_proposals[use_ephem_pk_idx]
            .enote_ephemeral_pubkey
            .clone();
    }
    ...
    for i in 0..selfsend_payment_proposals.len() {
        ...
        output_enote_proposals_out[output_idx] = match s_view_balance_dev {
            Some(s_view_balance_dev) => selfsend_payment_proposal.get_internal_output_proposal(
                s_view_balance_dev,
                tx_first_key_image.clone(),
                &other_enote_ephemeral_pubkey,
            )?,
            None => match k_view_dev {
                Some(k_view_dev) => selfsend_payment_proposal.get_special_output_proposal(
                    k_view_dev,
                    tx_first_key_image.clone(),
                    &other_enote_ephemeral_pubkey,
                )?,
    ...
    payment_proposal_order_out.sort_by(|a, b| {
        let a_output_idx = a.1
            + if a.0 {
                normal_payment_proposals.len()
            } else {
                0
            };
        let b_output_idx = b.1
            + if b.0 {
                normal_payment_proposals.len()
            } else {
                0
    ...
    apply_permutation_backwards(
        &mut PaymentProposalOrderSlice(payment_proposal_order_out),
...
pub fn get_coinbase_output_enotes(
    normal_payment_proposals: &[CarrotPaymentProposalV1],
    block_index: BlockIndex,
    output_coinbase_enotes_out: &mut [CarrotCoinbaseEnoteV1],
    payment_proposal_order_out: &mut [usize],
) -> Result<()> {
    // assert payment proposals numbers
    ...
    // assert output sizes
    if output_coinbase_enotes_out.len() != num_proposals
        || payment_proposal_order_out.len() != num_proposals
    {
        return Err(Error::new(ErrorKind::WrongOutputNumber));
    }
    ...
    for i in 0..num_proposals {
        ...
        output_coinbase_enotes_out[i] =
            normal_payment_proposals[i].get_coinbase_output_proposal(block_index)?;
    }
