...
fn get_normal_proposal_ecdh_parts(proposal: &CarrotPaymentProposalV1,
    input_context: &InputContext) -> Result<(EnoteEphemeralPubkey, MontgomeryECDH)>
{
    ...
    match make_carrot_uncontextualized_shared_key_sender(&enote_ephemeral_privkey,
...
fn get_output_proposal_parts(s_sender_receiver: &SenderReceiverSecret,
    ...
    -> Result<(AmountBlindingKey, AmountCommitment, OutputPubkey, EncryptedAmount, EncryptedPaymentId)>
{
    ...
    let onetime_address = make_carrot_onetime_address(destination_spend_pubkey,
        s_sender_receiver,
        &amount_commitment).ok_or(Error::new(ErrorKind::BadAddressPoints))?;
    
    // 4. a_enc = a XOR m_a
    let encrypted_amount = encrypt_carrot_amount(amount, s_sender_receiver, &onetime_address);
    
    // 5. pid_enc = pid XOR m_pid
    let encrypted_payment_id = encrypt_legacy_payment_id(payment_id, s_sender_receiver,
        &onetime_address);

    Ok((amount_blinding_factor, amount_commitment, onetime_address, encrypted_amount, encrypted_payment_id))
}


fn get_external_output_proposal_parts(s_sender_receiver_unctx: &MontgomeryECDH,
    destination_spend_pubkey: &AddressSpendPubkey,
    payment_id: &PaymentId,
    amount: Amount,
    enote_type: CarrotEnoteType,
    ...
    -> Result<(SenderReceiverSecret, AmountBlindingKey, AmountCommitment, OutputPubkey,
        EncryptedAmount, EncryptedPaymentId, ViewTag)>
{
    ...
    let (amount_blinding_factor, amount_commitment, onetime_address, encrypted_amount, encrypted_payment_id) = 
    ...
    let view_tag = make_carrot_view_tag(s_sender_receiver_unctx.0.as_bytes(), input_context, &onetime_address);

    Ok((s_sender_receiver, amount_blinding_factor, amount_commitment,
        onetime_address, encrypted_amount, encrypted_payment_id, view_tag))
}

fn get_enote_ephemeral_privkey(proposal: &CarrotPaymentProposalV1, input_context: &InputContext) -> EnoteEphemeralKey
{
...
fn try_resolve_selfsend_enote_ephemeral_pubkey<'a>(proposal: &'a CarrotPaymentProposalSelfSendV1,
    other_enote_ephemeral_pubkey: &'a Option<EnoteEphemeralPubkey>) -> Result<&'a EnoteEphemeralPubkey>
{
...
impl CarrotPaymentProposalV1 {
    pub fn get_enote_ephemeral_pubkey(&self, input_context: &InputContext) -> Result<EnoteEphemeralPubkey> {
        ...
        if self.destination.is_subaddress
        {
            ...
            make_carrot_enote_ephemeral_pubkey_subaddress(&enote_ephemeral_privkey,
                &self.destination.address_spend_pubkey).ok_or(Error::new(ErrorKind::BadAddressPoints))
        }
        else
        {
            // D_e = d_e B
            Ok(make_carrot_enote_ephemeral_pubkey_cryptonote(&enote_ephemeral_privkey))
        }
    }

    pub fn get_coinbase_output_proposal(&self,
        block_index: BlockIndex) -> Result<CarrotCoinbaseEnoteV1>
    {
        // 1. sanity checks
        ...
        let (enote_ephemeral_pubkey, s_sender_receiver_unctx) = get_normal_proposal_ecdh_parts(
            self,
            &input_context)?;
        ...
        let anchor_enc = encrypt_carrot_anchor(&self.randomness,
            &s_sender_receiver,
            &onetime_address);
    ...
    pub fn get_normal_output_proposal(&self, tx_first_key_image: KeyImage)
        -> Result<(RCTOutputEnoteProposal, EncryptedPaymentId)>
    {
        ...
        // 4. build the output enote address pieces
        let (s_sender_receiver, amount_blinding_factor, amount_commitment,
            onetime_address, amount_enc, encrypted_payment_id, view_tag) =
                get_external_output_proposal_parts(&s_sender_receiver_unctx,
                    &self.destination.address_spend_pubkey,
...
impl CarrotPaymentProposalSelfSendV1 {
    pub fn get_special_output_proposal<VI: ViewIncomingKeyDevice>(&self,
        k_view_dev: &VI,
        tx_first_key_image: KeyImage,
        other_enote_ephemeral_pubkey: &Option<EnoteEphemeralPubkey>) -> Result<RCTOutputEnoteProposal>
    {
        // 1. sanity checks
        ...
        // 3. D_e
        let enote_ephemeral_pubkey = try_resolve_selfsend_enote_ephemeral_pubkey(self,
        ...
        let (s_sender_receiver, amount_blinding_factor, amount_commitment,
            onetime_address, amount_enc, _, view_tag) =
                get_external_output_proposal_parts(&s_sender_receiver_unctx,
        ...
        let anchor_enc = encrypt_carrot_anchor(&janus_anchor_special,
            &s_sender_receiver,
            &onetime_address);
    ...
    pub fn get_internal_output_proposal<VB: ViewBalanceSecretDevice>(&self,
        ...
        other_enote_ephemeral_pubkey: &Option<EnoteEphemeralPubkey>) -> Result<RCTOutputEnoteProposal>
    {
        ...
        let s_sender_receiver = coerce_device_result(
            s_view_balance_dev.make_internal_sender_receiver_secret(&enote_ephemeral_pubkey, &input_context))?;

        // 5. build the output enote address pieces
        let (amount_blinding_factor, amount_commitment, onetime_address,
            amount_enc, _) = get_output_proposal_parts(&s_sender_receiver,
                &self.destination_address_spend_pubkey,
                &NULL_PAYMENT_ID,
                self.amount,
                self.enote_type,
