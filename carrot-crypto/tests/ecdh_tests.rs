...
fn ecdh_cryptonote_completeness() {
    ...
    let primary_address_view_pubkey = AddressViewPubkey::derive_primary_address_view_pubkey(&k_view);
    let k_ephem: EnoteEphemeralKey = gen_random();
    assert_ne!(k_view.as_bytes(), k_ephem.as_bytes());

    let enote_ephemeral_pubkey =
        EnoteEphemeralPubkey::derive_to_cryptonote_address(&k_ephem);

    let s_sr_sender = MontgomeryECDH::derive_as_sender(
        &k_ephem,
        &primary_address_view_pubkey,
    )
    .expect("make_carrot_uncontextualized_shared_key_sender");

    let s_sr_receiver = MontgomeryECDH::derive_as_receiver(
        &k_view,
...
fn ecdh_subaddress_completeness() {
    ...
    */
    let view_pubkey = AddressViewPubkey::derive_carrot_account_view_pubkey(&k_view,
            &spend_pubkey)
    ...
    let s_sr_sender =
        MontgomeryECDH::derive_as_sender(&k_ephem, &view_pubkey)
            .expect("make_carrot_uncontextualized_shared_key_sender");
