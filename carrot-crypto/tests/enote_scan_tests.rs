...
static BOB_CARROT_KEYS: LazyLock<MockKeys> =
    LazyLock::new(|| gen_random_with_params(AddressDeriveType::Carrot));
...
fn main_address_normal_scan_completeness() {
    ...
    let (enote_proposal, encrypted_payment_id) = proposal
        .get_normal_output_proposal(tx_first_key_image)
    ...
    ) = scan::try_scan_carrot_enote_external_receiver(
...
fn main_address_special_scan_completeness() {
    ...
    for enote_type in [CarrotEnoteType::Payment, CarrotEnoteType::Change].into_iter() {
        ...
        let enote_proposal = proposal
            .get_special_output_proposal(&keys.k_view_incoming, tx_first_key_image, &None)
...
fn main_address_internal_scan_completeness() {
    ...
    for enote_type in [CarrotEnoteType::Payment, CarrotEnoteType::Change].into_iter() {
        ...
        let enote_proposal = proposal
            .get_internal_output_proposal(&keys.s_view_balance, tx_first_key_image, &None)
        ...
        ) = scan::try_scan_carrot_enote_internal_receiver(
            &enote_proposal.enote,
            &keys.s_view_balance,
        )
        .expect("try_scan_carrot_enote_internal_receiver");
...
fn main_address_coinbase_scan_completeness() {
    ...
    let enote = proposal
        .get_coinbase_output_proposal(block_index)
        .expect("get_coinbase_output_proposal");
    ...
    let (
        ...
        recovered_sender_extension_t,
        recovered_address_spend_pubkey,
    ) = scan::try_scan_carrot_coinbase_enote_receiver(
        &enote,
        &s_sender_receiver_unctx,
        core::slice::from_ref(&keys.carrot_account_spend_pubkey),
    )
